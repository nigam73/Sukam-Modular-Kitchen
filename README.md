# Sukam Modular Kitchen

A modern and responsive website for **Sukam Modular Kitchen**, showcasing premium modular kitchen solutions, interior design services, wardrobes, home renovation, and customized furniture solutions in Kanpur.

🌐 **Live Website:** https://sukammodularkitchen.in/ :contentReference[oaicite:0]{index=0}

---

## 📌 About

Sukam Modular Kitchen is a professional interior design and modular kitchen company based in Kanpur. This website is designed to provide an elegant user experience while showcasing services, projects, blogs, catalogues, and contact information for potential customers. :contentReference[oaicite:1]{index=1}

---

## ✨ Features

- Modern Responsive Design
- SEO Friendly Structure
- Dynamic Service Pages
- Modular Kitchen Portfolio
- Interior Design Services
- Project Showcase
- Contact Form
- Blog Section
- Catalogue Download
- Sitemap & Robots.txt
- Fast Loading Website
- Mobile Friendly Layout

---

## 🛠️ Technologies Used

- HTML5
- CSS3
- Bootstrap
- JavaScript
- PHP
- jQuery

---

## 📂 Pages

- Home
- About Us
- Services
- Modular Kitchen
- Interior Design
- Wardrobe Design
- Bedroom Interior
- Living Room Interior
- False Ceiling
- Office Interior
- TV Unit Design
- Renovation & Remodeling
- Gallery
- Projects
- Catalogue
- Blogs
- Contact Us

---

## 🚀 Installation

```bash
git clone https://github.com/nigam73/Sukam-Modular-Kitchen.git

Move the project to your local server:

XAMPP
htdocs/

Start Apache and open:

http://localhost/Sukam/
```

---

## 📸 Live Preview

Website:
https://sukammodularkitchen.in/ :contentReference[oaicite:2]{index=2}

---

## 📞 Contact

**Sukam Modular Kitchen**

📍 Ram Puram, Shyam Nagar, Kanpur, Uttar Pradesh

📞 +91 63931 94453

📧 info@sukaminterior.in :contentReference[oaicite:3]{index=3}

---

## 👨‍💻 Developed By

**Nigam Kumar**

---

## 📄 License

This project is developed for Sukam Modular Kitchen. All design, branding, and content belong to their respective owners. Source code is shared only for portfolio and demonstration purposes.